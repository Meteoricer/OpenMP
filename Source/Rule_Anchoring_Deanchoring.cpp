...
void anchor_attatch()
{
	...
	while (attatch_anchor_iter != rule_structure.anchor_list.end())
	{
			...
			&& &*Grid[attatch_anchor_iter->anchor_grid_pointer->col_position][attatch_anchor_iter->anchor_grid_pointer->row_position].grid_polymer_unit_pointer!=&*rule_structure.polymer_unit_list_end.begin())//when it is empty
		{
			...
			{//do attatchment here
				...
				list<Polymer_Bundle>::iterator polymer_bundle_iter = attatch_anchor_iter->anchor_grid_pointer->grid_polymer_unit_pointer->polymer_cluster_pointer->cluster_bundle_pointer;
				for (int i = 0; i <= 3; i++)
				{
					if (polymer_bundle_iter->availabe_diffusion_direction[i] == true)
					{
	...
	if (attatch_temp_sum < attatch_anchor_rand)
	{
		cout << "attatch number error" << endl;
		system("pause");
	}
