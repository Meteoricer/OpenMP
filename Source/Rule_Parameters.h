...
public:
	...
	double polymer_diffusion_propensity = 0;
	...
	int T_sum;
	int D_sum;
	double average_T_distance;
	double average_D_distance;
	//int TT_annealing_sum=0;
	int step_num;
	int Z_ctp = 0;
	int reaction_mark;
	...
	int event_analysis = 0;
	ofstream event_output;
	vector<vector<int>> direction_record;
	vector<vector<int>> diffusion_statistics;
	vector<vector<int>> polymerize_statistics;
	vector<int> fragmentation_statistics;
	vector<int> depolymerization_statistics;
	
	vector<int> remove_statistics;
	vector<double> ps;
	vector<double> time;
	vector<double> count;
	vector<int> length_distribution;
	vector<double> average_length_per_column;
	vector<double> polymer_count;
	int bleached_Z = 0;
	int event_left = 0;
	int event_right = 0;
	int biased_previous = 0;
	int MinC = 0;
	...
	int row_num = 563;//563//200//the number in row direction,besiclly how many columns 
	int Anchor_num = 1000;//1000
	int Z = 15000;//8000//600//1000//4000
	...
	double anchoring_rate = 0.000005*scale_factor;//0.005//0.0005;
	double deanchoring_rate = 0.2;//10;//8;//4;//0.2;
	double TT_polymerize_vertical_plus_rate = 0.2*scale_factor;//0.03;//0.05
	...
	double TD_polymerize_vertical_plus_rate = 0.02*scale_factor;//0.01*scale_factor;//0.05
	...
	double diffusion_rate = 1000;//1000;//4000
	double emtpy_anchor_diffusion_rate = 1000;//1000;//0.1;
	double Ebdl = 0.3;//0.03//0.07
	double Ebdl_inverse = 0;
	double bundling_rate = 200;//50;//100;//500
	...
	double TT_annealing_rate = 1000;//2500;
	double TD_annealing_rate = 100;
	double fragmentation_multiplier = 10;
	...
	double TT_depolymerize_minus_rate = 0.01;//1;//8;//80#1
	...
	double TT_depolymerize_plus_rate = 0;// TT_depolymerize_minus_rate;//1;//8;//80#1
	...
	double attatch_rate = 100;//100;//100//0.000001;
	double both = 200;

	double hydrolysis_rate = 0.66 / 60 * 6.9 / 0.66*7.0 / 4.8*6.8 / 9 * 7 / 12 * 1 / 0.84*0.115 / 0.37;//7 / 0.66;
