...
void bundling()
{
	...
	{//if it is not the same bundle
		...
		for (int i = 0; i <= 3; i++)
		{
			if (lower_bundle_iter->availabe_diffusion_direction[i] == true)
			{
				rule_parameters.polymer_diffusion_propensity -= calculate_diffusion_rate(lower_bundle_iter, i);
				lower_bundle_iter->availabe_diffusion_direction[i] = false;
				lower_bundle_iter->available_polymer_diffusion_direction_sum--;
		...
		for (int i = 0; i <= 3; i++)
		{
			if (higher_bundle_iter->availabe_diffusion_direction[i] == true)
			{
				higher_bundle_iter->available_polymer_diffusion_direction_sum--;
				rule_parameters.polymer_diffusion_propensity -= calculate_diffusion_rate(higher_bundle_iter, i);
				higher_bundle_iter->availabe_diffusion_direction[i] = false;
...
void debundling()
{
	...
	{
		auto polymer_bundle_iter = large_polymer_unit_iter->polymer_cluster_pointer->cluster_bundle_pointer;
		...
		for (int i = 0; i <= 3; i++)
		{
			if (polymer_bundle_iter->availabe_diffusion_direction[i] == true)
			{
				rule_parameters.polymer_diffusion_propensity -= calculate_diffusion_rate(polymer_bundle_iter, i);
				directions[i] = 1;
				polymer_bundle_iter->availabe_diffusion_direction[i] = false;
				polymer_bundle_iter->available_polymer_diffusion_direction_sum--;
		...
		{//if they still belong to the same bundle
			...
			for (int i = 0; i <= 3; i++)
			{
				if (directions[i]==1)
				{
					polymer_bundle_iter->availabe_diffusion_direction[i] = true;
					rule_parameters.polymer_diffusion_propensity += calculate_diffusion_rate(polymer_bundle_iter, i);		
					polymer_bundle_iter->available_polymer_diffusion_direction_sum++;
