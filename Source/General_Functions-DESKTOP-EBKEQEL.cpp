...
void check_polymer_bundle_diffusable_direction(list<Polymer_Bundle>::iterator diffusion_bundle_iter,int check_mark)
{
	...
	//rule_parameters.polymer_diffusion_direction_sum -= diffusion_bundle_iter->available_polymer_diffusion_direction_sum;
...
void select_and_execute()
{
	...
	if (effect_rate == 1)
	{
		if (rule_parameters.TTT_sum_total + rule_parameters.TTD_sum_total + rule_parameters.DTD_sum_total != 0)
		{
				...
				(rule_parameters.TTT_sum_total + rule_parameters.TTD_sum_total + rule_parameters.DTD_sum_total) > rule_parameters.hydrolysis_rate)
			{
				...
				rule_parameters.TTD_hydrolysys_rate = rule_parameters.TTT_hydrolysys_rate * 5;//1;//20;//20;// 100;//100;
				rule_parameters.DTD_hydrolysys_rate = rule_parameters.TTD_hydrolysys_rate * 5;//2;//40;//10;// 100;//100;
			}
			else
			{
	...
	rule_parameters.ps[rule_diffusion_polymer] = rule_parameters.polymer_diffusion_direction_sum*rule_parameters.diffusion_rate;
		...
		for (int j = 0; j < rule_parameters.row_num; j++)
		{
			if (Grid[i][j].grid_anchor_pointer != rule_structure.anchor_list.end())
			{
				...
				else
				{
					...
					else if (Grid[i][j].grid_polymer_unit_pointer->Hydrolysis_mark == D)
					{
					fout << D << " ";
					rule_parameters.D_sum++;
					count++;
					}*/
					if (Grid[i][j].grid_polymer_unit_pointer->polymer_cluster_pointer->polarize == direction_first)
					{
					...
					else if (Grid[i][j].grid_polymer_unit_pointer->polymer_cluster_pointer->polarize == direction_second)
					{
						if (Grid[i][j].grid_polymer_unit_pointer->Hydrolysis_mark == T)
						{
							fout << zero_count << " " << ATN << " ";
					...
					/*stringstream a;
					a<< &*(Grid[i][j].grid_polymer_unit_pointer->polymer_cluster_pointer->cluster_bundle_pointer);
					int b;
					a >> b;
					fout <<dec<<b<<" ";
					a.clear();*/
					/*if ((&*Grid[i][j].grid_polymer_unit_pointer)->anchor_pointer != rule_structure.anchor_list.end())
					{
					fout << 9 << " ";
					count++;
					}
					else if((&*Grid[i][j].grid_polymer_unit_pointer)->polymer_cluster_pointer->direction == vertical)
					{
					fout << vertical << " ";
					count++;
					}
					else if ((&*Grid[i][j].grid_polymer_unit_pointer)->polymer_cluster_pointer->direction == horizontal)
					{
					fout << horizontal << " ";
					count++;
					}*/
...
void check()
{
	...
	int diffusion_direction_temp_sum = 0;
	...
	while (polymer_bundle_iter != rule_structure.polymer_bundle_list.end())
	{
		...
		diffusion_direction_temp_sum += temp_sum;
		if (temp_sum != polymer_bundle_iter->available_polymer_diffusion_direction_sum)
		{
			std::cout << "error: diffusion direction sum is not the same as sum up" << endl;
			system("pause");
	...
	if (diffusion_direction_temp_sum != rule_parameters.polymer_diffusion_direction_sum)
	{
		cout << "polymer diffusion direction sum doesn't match" << endl;
		system("pause");
	}
...
void check_sumps()
{
	...
	for (int i = 0; i < num_of_rules; i++)
	{
		...
		{
			std::cout << "error: negative reaction parameter" << endl;
			system("pause");
		}
	}
